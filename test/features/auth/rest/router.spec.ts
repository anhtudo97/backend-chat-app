...
const refreshToken = "bleleele";
const accessToken = "dahdahhhh";
const userID = "userIDDDD";

describe("/auth/refresh_token", () => {
  it("should return 401 if no cookie is set", async () => {
    // act
    const req = request(app).get("/auth/refresh_token");
    const res = await req;
    // assert
  ...
  it("should return 401 if the token in the cookie is invalid", async () => {
    // arrange
    when(MockTokens.verifyRefreshToken(anything())).thenThrow(
  ...
  it("should return 200 with the access token if the access token is valid", async () => {
    // arrange
    when(MockTokens.verifyRefreshToken(anything())).thenReturn(userID);
...
describe("/auth/logout", () => {
  beforeAll(() => {
    app.get("/mock_login", (req, res) => {
      res.cookie("veryGoodCookie", refreshToken, {
        path: "/auth",
        httpOnly: true,
