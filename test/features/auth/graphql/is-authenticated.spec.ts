...
import isAuthenticated from "../../../../src/features/auth/graphql/is-authenticated";
...
beforeEach(() => resetCalls(MockTokens));

it("should should throw an authentication error if no authorization header was sent", () => {
  expect(act).toThrow(AuthenticationError);
});

it("should should throw an authentication error the authorization header has no token", () => {
...
describe("When the authorization header exists and contains a token", () => {
  ...
  it("should throw an authentication error if the verification fails", () => {
    // arrange
    when(MockTokens.verifyAccessToken(anything())).thenThrow(
  ...
  it(`should set the userID in the context, and return calling the next 
