...
import AuthDataSource from "../../../../src/features/auth/data/auth-data-source";
import { Tokens } from "../../../../src/features/auth/data/tokens";
import { Response } from "express";
import { AxiosError } from "axios";
import { mockAuthProviderUser, mockPrismaAuthUser } from "../../../mock-object";
...
beforeEach(() => {
  when(MockGoogleApi.getGoogleUser(anything())).thenResolve(
    mockAuthProviderUser
  );
  when(MockAuthDS.findOrCreateAuthUser(anything())).thenResolve(
    mockPrismaAuthUser
  );
...
describe("loginWithGoogle", () => {
  ...
  it("should throw INTERNAL_SERVER_ERROR if authDS fails", async () => {
    // arrange
    when(MockAuthDS.findOrCreateAuthUser(anything())).thenReject(
      new Error("Blah")
    );
    // act
    const thrown = await getThrownError();
  ...
  it("should throw INTERNAL_SERVER_ERROR if generating access token fails", async () => {
    // arrange
    when(MockTokens.generateAccessToken(anything())).thenThrow(
      new Error("Blah")
    );
    // act
    const thrown = await getThrownError();
  ...
  it("should throw INTERNAL_SERVER_ERROR if generating refresh token fails", async () => {
    // arrange
    when(MockTokens.generateRefreshToken(anything())).thenThrow(
      new Error("Blah")
    );
    // act
    const thrown = await getThrownError();
  ...
  it("should return the right response and set the cookie if everything succeeds", async () => {
    // act
