...
beforeAll(() => {
  ...
  when(MockSigner.verify(anything(), anything())).thenReturn({ userID });
});
...
it("should generate and verify access tokens", () => {
  ...
  // assert again
  verify(
    MockSigner.verify(codedToken, process.env.ACCESS_TOKEN_SECRET!)
  ).once();
...
it("should generate and verify refresh tokens", () => {
  ...
  // assert
  verify(
    MockSigner.sign(deepEqual({ userID }), process.env.REFRESH_TOKEN_SECRET!)
  ).once();
  expect(codedToken).toBe(token);
  // act again
  ...
  // assert again
  verify(
    MockSigner.verify(codedToken, process.env.REFRESH_TOKEN_SECRET!)
  ).once();
  expect(decodedUserID).toBe(userID);
});
