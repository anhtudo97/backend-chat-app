...
import { mockPrismaAuthUser } from "../../../mock-object";
...
describe("findOrCreateAuthUser", () => {
  it("should return the auth user if it exists", async () => {
    // arrange
    when(MockAuthUserDelegate.findUnique(anything())).thenResolve(
      mockPrismaAuthUser
    );
    // act
    ...
    // assert
    verify(
      MockAuthUserDelegate.findUnique(deepEqual({ where: { email } }))
    ).once();
    verify(MockAuthUserDelegate.create(deepEqual({ data: { email } }))).never();
    expect(result).toStrictEqual(mockPrismaAuthUser);
  });

  it("should create then return the auth user if it doesn't exist", async () => {
    // arrange
    when(MockAuthUserDelegate.findUnique(anything())).thenResolve(null);
    when(MockAuthUserDelegate.create(anything())).thenResolve(
      mockPrismaAuthUser
    );
    // act
    const result = await authDS.findOrCreateAuthUser(email);
