...
import BadgeDataSource from "../../../../src/features/badge/data/badge-data-source";
import { mockPrismaBadges, mockTheDate } from "../../../mock-object";
...
describe("getUserBadges", () => {
  ...
  it("should create the badges if they don't exist, then return them", async () => {
    ...
    Object.values(BadgeName).forEach((badgeName) => {
      verify(
        MockBadgeDelegate.create(
          deepEqual({ data: { userID, badgeName, lastOpened: date } })
        )
      ).once();
    });
  });
