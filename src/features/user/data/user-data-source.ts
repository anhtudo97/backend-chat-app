...
import { GetUserArgs, Me, User } from "../graphql/types";
...
export default class UserDataSource {
  ...
  constructor(@inject(TYPES.PrismaCLient) prima: PrismaClient) {
    this._prisma = prima;
  }
  ...
  async getUser({ id, username }: GetUserArgs): Promise<User | null> {
  ...
  async isUsernameTaken(username: string) {
    const user = await this._prisma.user.findUnique({ where: { username } });
    return user !== null;
  }
  ...
  async findUsers(
    ...
    return users.map((u: User) => UserDataSource._getGraphQLUser(u));
  }
